 ...
 */
public class Columns2PrimaryKeyProcessorTemp extends AbstractEventProcessor {
                ...
                for(EventColumn eventColumn:eventColumns ){
                    if(!eventColumn.isUpdate() || eventData.getEventType() == EventType.DELETE){
                        if(!Strings.isNullOrEmpty(eventColumn.getColumnValue())){
                            ...
                            if("memo".equalsIgnoreCase(eventColumn.getColumnName()) && !Strings.isNullOrEmpty(eventColumn.getColumnValue())){
                                System.out.println(eventColumn);
                            }
